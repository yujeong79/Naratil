...
public class SimilarityTargetReader implements ItemReader<List<BidNotice>>, ItemStream {
    ...
    @Override
    public List<BidNotice> read() {
        if (cursor == null || !cursor.hasNext()) return null;

        List<BidNotice> batch = new ArrayList<>(BATCH_SIZE);
        int count = 0;
    ...
    private Document buildQuery(){
        ...
        return new Document("$and", Arrays.asList(
                ...
                new Document("similarNtces", new Document("$exists", false)) // 아직 선정 안된 것만
        ));
    }
