...
@Slf4j
@Component
public class SimilarNtcesEmptyWriter implements ItemWriter<List<BidNtce>> {
    ...
    public SimilarNtcesEmptyWriter(MongoTemplate mongoTemplate, SimilarityApiClient apiClient, MongoCollectionChecker collectionChecker) {
    ...
    @Override
    public void write(Chunk<? extends List<BidNtce>> chunk) {
        List<BidNtce> notices = chunk.getItems().stream()
                .flatMap(List::stream)
                .toList();
        ...
        // 200개씩 나눔
        List<List<BidNtce>> batches = partition(notices, BATCH_SIZE);
        log.debug("[Writer] 유사 공고 전체 {}개 공고 → {}개 배치로 분할 완료", notices.size(), batches.size());


        for (int i = 0; i < batches.size(); i++) {
            List<BidNtce> batch = batches.get(i);
     ...
     * BidNotice -> NtceComparison 변환
     */
    private NtceForComparison toComparison(BidNtce notice) {
    ...
    private Document toDocument(SimilarNtceItem item, Map<Long, String> idToName) {
        ...
        doc.append("collectionNoLimit", true);
        return doc;
    }
